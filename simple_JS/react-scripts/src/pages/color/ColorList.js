...
const TABLE_HEAD = [
  ...
  { id: 'name', label: 'Tên màu sắc', alignRight: false },
];
...
export default function UserList() {
  ...
  useEffect(() => {
    fetch(`${link}/color/get`, requestOptions)
  ...
  const NewDetailSchema = Yup.object().shape({
    name: Yup.string().required('Bạn chưa nhập tên màu sắc!'),
  });
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await fetch(`${link}/color/checkDuplicatedColor?color=${info.name}`)
        .then((response) => response.json())
        .then((rs) => {
          if (rs === true) {
            console.log('rs1234', rs);
            fetch(`${link}/color/add`, {
              method: 'POST',
          ...
          if (rs === false) {
            enqueueSnackbar(`Màu sắc đã tồn tại!`, { variant: 'warning' });
          }
        });
    } catch (error) {
      console.error(error);
  ...
  return (
    <Page title="Danh mục màu sắc">
      ...
      <Dialog open={openDialogAdd} fullWidth maxWidth="xs" onClose={() => setOpenDialogAdd(false)}>
        <DialogTitle>Thêm mới màu sắc</DialogTitle>
        <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
          <DialogContent>
            <Stack spacing={1}>
              <RHFTextField name="name" label="Màu sắc" />
            </Stack>
          </DialogContent>
          <DialogActions>
